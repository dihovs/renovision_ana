...
/**
 * The conversation itself — header, disclaimer, message list, option rows,
 * calculator, lead form, composer. Everything that makes the estimator work
 * lives here; the shells in ChatWidget.tsx only decide where it sits.
 *
 * "floating" is the corner overlay that has always been mounted in the root
 * layout. "inline" is the same conversation dropped into normal page flow on
 * /estimation, where the tool IS the page rather than something you click a
 * bubble to reach. The two differ in chrome and in how the list scrolls —
 * never in behaviour.
 */
export type ChatVariant = "floating" | "inline";
...
/** Card chrome. Only the frame changes between variants; the contents don't. */
const SHELL_CLASS: Record<ChatVariant, string> = {
  floating:
    "relative flex h-full w-full flex-col overflow-hidden bg-white shadow-2xl sm:h-[720px] sm:max-h-[88vh] sm:w-[26rem] sm:rounded-3xl sm:border sm:border-black/10",
  // Shorter on phones so the card doesn't swallow the whole screen; taller on
  // desktop where there's room to actually read the conversation. Header,
  // disclaimer strip and composer take ~250px of this on a narrow screen, so
  // going much below 560 leaves the message list too cramped to follow.
  inline:
    "relative flex h-[560px] w-full flex-col overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm sm:h-[600px]",
};
...
export default function ChatConversation({
  ...
  variant: ChatVariant;
  /** Floating only. Omitted inline, where the conversation is always open. */
  onClose?: () => void;
}) {
  ...
  //
  // Inline can't use scrollIntoView: the card sits in page flow, so scrolling a
  // bubble into view would drag the whole window down mid-conversation. There
  // the list is scrolled directly and the page stays where the reader left it.
  useEffect(() => {
    if (variant === "inline") {
      const list = listRef.current;
      if (!list) return;
      const top = calculatorRef.current ? calculatorRef.current.offsetTop : list.scrollHeight;
  ...
  }, [messages, step, track, estimate, variant]);
  ...
  return (
    <div className={SHELL_CLASS[variant]}>
      <div
      ...
      >
        ...
        <div className="relative flex items-center justify-between">
          ...
          {/* Inline has nowhere to close to — the conversation is the page. */}
          {onClose && (
            <button
              type="button"
              onClick={onClose}
