  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-3 text-charcoal/70">{t.howItWorks.subtitle}</p>
      </div>

      <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
        {steps.map(({ icon: Icon, title, desc }, i) => (
          <div key={title} className="relative text-center">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-brand-blue text-white">
              <Icon className="h-7 w-7" />
            </div>
            <span className="mt-3 block text-xs font-bold uppercase tracking-wide text-brand-green">
              Step {i + 1}
            </span>
            <h3 className="mt-1 font-heading text-lg font-bold text-brand-blue">{title}</h3>
